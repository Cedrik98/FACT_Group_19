...
class GA(SearchMethod):
    ...
    def create_population(self, doc, population_size):
        ...
        transformed_texts = self.get_transformations(
            doc.attacked_text, original_text=doc.attacked_text
        )
        ...
        for i in range(population_size):
            ...
            d = PopulationMember(
                ...
                doc,
                attributes={
                    "available_word_replacements": np.copy(
                        available_word_replacements
                    )
                },
            )
            # Mutate
    ...
    def mutate(self, doc, original_result):
        ...
        non_zero_indices = np.count_nonzero(
            doc.attributes["available_word_replacements"]
        )
        ...
        while iterations < non_zero_indices:
            ...
            new_results, self.search_over = self.get_goal_results(
                transformed_texts
            )

            # Calculate comparative fitness values for mutated candidates against original document
            candidate_fitnesses = (
                torch.Tensor([candidate.score for candidate in new_results])
                - doc.result.score
            )
            ...
            if len(candidate_fitnesses) and candidate_fitnesses.max() > 0:
                ...
                doc = PopulationMember(
                    transformed_texts[best_fitness_index],
                    new_results[best_fitness_index],
                    attributes={
                        "available_word_replacements": available_word_replacements
                    },
                )
    ...
    def crossover(self, parent1, parent2, original_text):
        ...
        if self.crossover_type == "uniform":
            ...
            words_to_replace = []
            available_word_replacements = np.copy(
                parent1.attributes["available_word_replacements"]
            )
            ...
            for i in range(parent1.num_words):
                if random.random() < 0.5:
                    ...
                    available_word_replacements[i] = parent2.attributes[
                        "available_word_replacements"
                    ][i]

            child = parent1.attacked_text.replace_words_at_indices(
        ...
        child.attack_attrs["modified_indices"] = (
            parent1_chromosome.attack_attrs["modified_indices"]
            - replaced_indices
        ) | (
            parent2_chromosome.attack_attrs["modified_indices"]
            & replaced_indices
        )
        ...
        if "last_transformation" in parent1_chromosome.attack_attrs:
            child.attack_attrs[
                "last_transformation"
            ] = parent1_chromosome.attack_attrs["last_transformation"]
        elif "last_transformation" in parent2_chromosome.attack_attrs:
            child.attack_attrs[
                "last_transformation"
            ] = parent2_chromosome.attack_attrs["last_transformation"]
        ...
        if "last_transformation" in child.attack_attrs:
            ...
            passed_constraints = self.check_constraints(
                child, previous_text, original_text=original_text
            )
        ...
        if passed_constraints:
            ...
            return PopulationMember(
                child, result=new_results[0], attributes=attributes
            )
    ...
    def perform_search(self, doc):
        ...
        for i in range(self.generations):
            print("\nGeneration: ", i)
            population = sorted(
                population, key=lambda x: x.result.score, reverse=True
            )
            ...
            if (
                population[0].result.goal_status
            ...
            if self.parent_selection == "roulette":
                ...
                parent1_indicies = random.choices(
                    range(self.population_size - 1), k=self.population_size
                )
                parent2_indicies = random.choices(
                    range(self.population_size - 1), k=self.population_size
                )
                ...
                population = (
                    population[0 : int(self.population_size * factor)]
                    + population[0 : int(self.population_size * factor)]
                )
            ...
            for index in range(self.population_size - 1):
                child = self.crossover(
                    population[parent1_indicies[index]],
                    population[parent2_indicies[index]],
                    doc.attacked_text,
                )
