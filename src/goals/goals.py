...
class ADV_XAI_GF(ClassificationGoalFunction):
    ...
    def _compareFeatureCounts(self, attacked_text):
        ...
        for i in range(len(self.features[0])):
            featureCountsTransformed.append(
                attacked_text.text.count(self.features[0][i])
            )
    ...
    def RBO(self, list1, list2, p):
        ...
        return (
            (len(set(list1) & set(list2)) / comparisonLength)
            * math.pow(p, comparisonLength)
        ) + (((1 - p) / p) * summation)
    ...
    def kendall(self, attacked_text, explanation_dataframe):
        ...
        explanation_dataframe = explanation_dataframe.sort_values(
    ...
    def _is_goal_complete(self, model_output, attacked_text):
        ...
        if self.basePrediction != perturbedExplanation[1]:
            if self.logger:
                print(
                    "FAILED! Base prediction class must be the same as the attacked prediction class",
                )
            return False
        ...
        if "newly_modified_indices" in attacked_text.attack_attrs:
            ...
            for j in modified_index:
                ...
                if (
                    ...
                    and to_w_j.lower()
                    in targets.get("feature")[: self.featureSelector].values
                ):
                    if self.logger:
                        print(
                            f"FAILED! the replacement ``{from_w}``->``{to_w}`` force ``{to_w_j}`` appears in top_n {self.featureSelector} but was not in the orgiginal text"
                        )
                    # print(targets)
                    return False
        ...
        if self.RBO_flag:
            ...
            for i in range(len(self.features[0])):  # check in the top features
                ...
                if not (
                    self.features[2][i]
                    >= targets.index[targets.feature == self.features[0][i]][0]
                ):
                    decreaseFlag = True
        ...
        baseList = self.baseExplanationDataframe.get(
            "feature"
        ).values  # assume that they are already sorted

        targetList = self.baseExplanationDataframe.get(
            "feature"
        ).values  # assume that they are already sorted
                ...
                if self.logger:
                    print(
                        "FAILED! Explanation still too similar, {} {}".format(
                            self.tempScore, self.success_threshold
                        )
                    )
                return False
    ...
    def _should_skip(self, model_output, attacked_text):
        if model_output.argmax() != self.ground_truth_output:
            if self.logger:
                print(
                    "skipping because the model's prediction was wrong",
                    model_output.argmax(),
                    "vs",
                    self.ground_truth_output,
                )
    ...
    def _get_score(self, model_output, _):
        if self.RBO_flag:
            ...
            if self.tempScore is not None:
                if (
                    self.tempScore > self.baseCOM
                ):  # perturbed com on the right side of the base com
                    return (self.tempScore - self.baseCOM) / (
                        (self.initial_attacked_text_length - 1) - self.baseCOM
                    )
                elif (
                    self.tempScore < self.baseCOM
                ):  # perturbed com on the left side of the base com
