...
from utils import ADV_XAI_RBO, RANDOM_BASELINE
...
if __name__ == "__main__":
    ...
    def generate_attacker(ATTACK_CLASS, args, custom_seed=None, greedy_search=True):
        ...
        attacker = textattack.Attacker(
            attack, textattack.datasets.Dataset([]), attack_args
        )
    ...
    if args.method == "xaifooler":
        ...
        attacker1 = generate_attacker(
            ADV_XAI_RBO, args, custom_seed=np.random.choice(1000)
        )
        attacker2 = generate_attacker(
            ADV_XAI_RBO, args, custom_seed=np.random.choice(1000)
        )

    elif args.method == "random":
        attacker = generate_attacker(
            RANDOM_BASELINE, args, custom_seed=None, greedy_search=True
        )

    elif args.method == "truerandom":
        attacker = generate_attacker(
            RANDOM_BASELINE, args, custom_seed=None, greedy_search=False
        )
