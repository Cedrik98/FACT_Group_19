...
def check_condition1(result, df1, df2, configs):
    ...
    if modified_index:
        for j in modified_index:
            ...
            if (
                to_w_j.lower() not in df1.get("feature")
                and to_w_j.lower() in df2.get("feature").values[: configs["top_n"]]
            ):
                print(
                    f"FAILED! `{to_w_j}`` appears in top_n but was not in the orgiginal text"
                )
                return False
    return True
...
def eval(args, filename=None, use=None):
    ...
    for item in results:
        ...
        if not check_condition2(exp1, exp2) and not check_condition1(
            result, df1, df2, configs
        ):
            num_errors += 1
            continue
        ...
        if result:
            modified_index = result.perturbed_result.attacked_text.attack_attrs[
                "modified_indices"
            ]
            replacement_words = [
                result.perturbed_result.attacked_text.words[i] for i in modified_index
            ]
            num_replacements.append(len(replacement_words))

        rboOutput = RBO(
            targetList[: configs["top_n"]], baseList[: configs["top_n"]], p=1.0
        )
        ...
        rank2 = (
            ...
            .reindex(df1["feature"])[: configs["top_n"]]["rank"]
            .values
        )
