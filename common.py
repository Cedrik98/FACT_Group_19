...
def p_generator(p, d):
    ...
    return (
        1
        - math.pow(p, d - 1)
        + (((1 - p) / p) * d * (np.log(1 / (1 - p)) - sum_series(p, d - 1)))
    )
    ...
    for i in range(1, 100, 1):
        ...
        if abs(output - top_n_mass) < 0.01:
            print(
                ...
                " for ",
                output * 100,
                "% mass to be assigned to the top ",
                n,
                " features.",
            )
            break
...
def load_args():
    ...
    parser.add_argument("--rbo_threshold", type=int, default=10)
...
def load_dataset_custom(DATASET_NAME, seed):
    ...
    if DATASET_NAME == "gb":
        ...
        dataset = load_dataset(DATASET_NAME, split="train").filter(
            lambda example: example["class"] in set([0, 1])
        )
...
def generate_explanation_single(
    self, document, custom_n_samples=None, debug=False, return_explainer=False
):
