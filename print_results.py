...
if __name__ == "__main__":
    checkpoints = [
        (
            "L2",
            "L2_dataset-gb_model-{}-md_gender_bias-saved_s-512_k-10_n-2_-sr-1500_threshold-0.5_seed-1212_modifyrate-0.2_RBOrate-0.46",
        ),
        (
            "XAIFooler0.46",
            "dataset-gb_model-{}-md_gender_bias-saved_s-512_k-10_n-2_-sr-1500_threshold-0.5_seed-1212_modifyrate-0.2_RBOrate-0.46",
        ),
        (
            "XAIFooler0.32",
            "dataset-gb_model-{}-md_gender_bias-saved_s-512_k-10_n-2_-sr-1500_threshold-0.0_seed-1212_modifyrate-0.2_RBOrate-0.32",
        ),
    ]
        ...
        for model in ["bert-base-uncased", "distilbert-base-uncased", "roberta-base"]:
            try:
                ...
                with open(args_file, "r") as f:
                    ...
                    df = eval(
                        args, filename=f"./results/{checkpoint.format(model)}/", use=use
                    )
                    df["Method"] = name
                    df["Model"] = model
                    df = df[keep_cols]
